...
// Função que cria uma borda de "!" na tela e informa que a energia da fábrica está no limite
void ssd1306_draw_alert_message(ssd1306_t *ssd){
  ssd1306_fill(ssd,false);
  ssd1306_draw_string(ssd," !!!!!!!!!!!!!!",0,2);
  ssd1306_draw_string(ssd," !",0,10);
  ssd1306_draw_string(ssd," !",104,10);
  ssd1306_draw_string(ssd," !",0,18);
  ssd1306_draw_string(ssd," !",104,18);
  ssd1306_draw_string(ssd," !",0,26);
  ssd1306_draw_string(ssd," !",104,26);
  ssd1306_draw_string(ssd," !",0,34);
  ssd1306_draw_string(ssd," !",104,34);
  ssd1306_draw_string(ssd," !",0,42);
  ssd1306_draw_string(ssd," !",104,42);
  ssd1306_draw_string(ssd,"ENERGIA",42,10);
  ssd1306_draw_string(ssd,"  QUASE",40,20);
  ssd1306_draw_string(ssd,"NO",55,30);
  ssd1306_draw_string(ssd,"LIMITE",40,40);
  ssd1306_draw_string(ssd," !!!!!!!!!!!!!!",0,50);       
}


// Função simples somente para criar uma borda na tela do display oled e exibir a quantidade de setores ativos no momentos
void ssd1306_show_numbers_of_sectors_on(ssd1306_t * ssd,int numberOfSectors){
  char ch[2]; 
  sprintf(ch, "%d", numberOfSectors);
  ssd1306_fill(ssd,false);
  ssd1306_rect(ssd,3,3,122,60,true,false);
  ssd1306_rect(ssd,4,4,120,58,true,false);
  ssd1306_rect(ssd,5,5,118,56,true,false);
  ssd1306_draw_string(ssd," Quantidade",18,10);
  ssd1306_draw_string(ssd,"Setores",36,20);
  ssd1306_draw_string(ssd,"Energizados ",22,30);
  ssd1306_draw_string(ssd,ch,55,40);
}